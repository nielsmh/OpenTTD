...
const ScoreInfo _score_info[] = {
	...
	{   10000,  50, SCOREUNIT_MONEY_K, STR_PERFORMANCE_DETAIL_MONEY,      STR_PERFORMANCE_DETAIL_MONEY_TOOLTIP     }, // SCORE_MONEY
	{    -250,  50, SCOREUNIT_MONEY_K, STR_PERFORMANCE_DETAIL_LOAN,       STR_PERFORMANCE_DETAIL_LOAN_TOOLTIP      }, // SCORE_LOAN
...
int UpdateCompanyRatingAndValue(Company *c, bool update)
{
	...
	{
		if (c->money > 0) {
			_score_part[owner][SCORE_MONEY] = c->money / 1000;
		}
	}
	...
	int score = 0;
	{
		...
		for (ScoreID i = SCORE_BEGIN; i < SCORE_END; i++) {
			...
			/*  Check the score */
			int64 needed = _score_info[i].needed;
			int64 value = _score_part[owner][i];
			if (needed < 0) {
				/* Inverse requirement */
				needed = -needed;
				value = needed - value;
